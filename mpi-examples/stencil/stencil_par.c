...
void printarr_par(int iter, double *array, int size, int px, int py, int rx, int ry, int bx, int by,
                  int offx, int offy, int (*ind)(int, int, int), MPI_Comm comm, int nproc)
{
    ...
    // make WRK directory
    if(myrank == 0) mkdir("WRK", S_IRWXU | S_IRWXG | S_IRWXO);

    switch (nproc) {
        case 1:
            if(myrank == 0) mkdir("WRK/1", S_IRWXU | S_IRWXG | S_IRWXO);
            snprintf(fname, 128,"./WRK/1/output-%i.bmp", iter);
            break;
        case 2:
            if(myrank == 0) mkdir("WRK/2", S_IRWXU | S_IRWXG | S_IRWXO);
            snprintf(fname, 128,"./WRK/2/output-%i.bmp", iter);
            break;
        case 4:
            if(myrank == 0) mkdir("WRK/4", S_IRWXU | S_IRWXG | S_IRWXO);
            snprintf(fname, 128,"./WRK/4/output-%i.bmp", iter);
            break;
        case 8:
            if(myrank == 0) mkdir("WRK/8", S_IRWXU | S_IRWXG | S_IRWXO);
            snprintf(fname, 128,"./WRK/8/output-%i.bmp", iter);
            break;
        case 16:
            if(myrank == 0) mkdir("WRK/16", S_IRWXU | S_IRWXG | S_IRWXO);
            snprintf(fname, 128,"./WRK/16/output-%i.bmp", iter);
            break;
    }
